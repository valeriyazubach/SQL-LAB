...
USE company;
SELECT 
    LPAD(product_id, 4, '0') AS 'Ідентифікатор',
    CONCAT(p.manufacture, ' :: ', SUBSTRING_INDEX(p.product_name, '/', -1) ) AS 'Назва товару',
    UPPER( CONCAT(p.product_type, ' - ', p.category)) AS 'Категорія'
FROM
    product p
ORDER BY 
    p.manufacture; 
